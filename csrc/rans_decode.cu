...
// ─── Interleaved dual-stream pair decode ─────────────────────────────
//
// Explicitly interleaves the two streams' decode steps: issue both
// __ldg reads BEFORE consuming either result. This puts 2 L1 reads
// in flight simultaneously, giving the L1 ~12 more cycles to return
// data (2 slot computes + 2 LDG issues ≈ 6 cycles before first
// result is needed, vs ~1 cycle with sequential decode_step calls).
//
// No register pressure increase — same 2 streams, same context.

__global__ void rans_decode_fp8_pair_interleaved_dump_kernel(
    const uint32_t* __restrict__ compressed_u32,
    const int32_t*  __restrict__ block_offsets,
    const uint32_t* __restrict__ final_states,
    const uint32_t* __restrict__ sfc_global,
    ...
    int64_t n_streams)
{
    ...
    for (int64_t i = 0; i < n_pairs; i++) {
        // ── Phase 1: compute slots + issue both __ldg reads ──
        uint32_t slot_a = 0, slot_b = 0;
        uint32_t entry_a = 0, entry_b = 0;

        if (have_a) {
            slot_a = A.x & (rans_gpu_pair::M_SIZE - 1);
            entry_a = __ldg(&sfc_global[slot_a]);  // issue LDG A
        }
        if (have_b) {
            slot_b = B.x & (rans_gpu_pair::M_SIZE - 1);
            entry_b = __ldg(&sfc_global[slot_b]);  // issue LDG B
        }
        // Both reads now in flight — L1 processes them concurrently.

        // ── Phase 2: consume A's result ──
        uint8_t sym_a = 0;
        if (have_a) {
            sym_a    = (uint8_t)(entry_a & 0xFF);
            uint32_t f = (entry_a >> 8) & 0xFFF;
            uint32_t c = (entry_a >> 20) & 0xFFF;
            A.x = (A.x >> rans_gpu_pair::M_LOG) * f + (slot_a - c);

            // Branchless renorm
            bool need = (A.x < rans_gpu_pair::L_LOW);
            if (need && A.buf_avail == 0) {
                A.slab_idx -= 2;
                int64_t off = A.block_base_u32
                    + (int64_t)A.slab_idx * blockDim.x + tid;
        ...
        // ── Phase 3: consume B's result ──
        uint8_t sym_b = 0;
        if (have_b) {
            sym_b    = (uint8_t)(entry_b & 0xFF);
            uint32_t f = (entry_b >> 8) & 0xFFF;
            uint32_t c = (entry_b >> 20) & 0xFFF;
            B.x = (B.x >> rans_gpu_pair::M_LOG) * f + (slot_b - c);

            bool need = (B.x < rans_gpu_pair::L_LOW);
            if (need && B.buf_avail == 0) {
                B.slab_idx -= 2;
                int64_t off = B.block_base_u32
                    + (int64_t)B.slab_idx * blockDim.x + tid;
        ...
        // ── Phase 4: compose fp8 + digest ──
        if (have_a) {
            uint8_t sm = sign_mantissa[i * n_streams + tid_a];
            uint8_t e0 = sym_a >> 4, e1 = sym_a & 0xF;
        ...
        if (have_b) {
            ...
            uint8_t e0 = sym_b >> 4, e1 = sym_b & 0xF;
            uint8_t s0 = sm & 0xF, s1 = sm >> 4;
            digest ^= ((s0&8)<<4)|(e0<<3)|(s0&7);
            digest ^= ((s1&8)<<4)|(e1<<3)|(s1&7);
        }
    ...
    int64_t n_fp8_per_stream, torch::Tensor freqs)
{
    ...
    auto digest = torch::empty(
        {blocks * threads},
        torch::TensorOptions().dtype(torch::kUInt8).device(compressed.device()));
    rans_decode_fp8_pair_interleaved_dump_kernel<<<blocks, threads>>>(
        reinterpret_cast<const uint32_t*>(compressed.data_ptr<uint8_t>()),
        block_offsets.data_ptr<int32_t>(),
        reinterpret_cast<const uint32_t*>(final_states.data_ptr<int32_t>()),
        reinterpret_cast<const uint32_t*>(sfc_gpu.data_ptr<int32_t>()),
