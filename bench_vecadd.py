...
def bench_memcpy(n_bytes: int) -> dict[str, float]:
    a = torch.randint(0, 256, (n_bytes,), dtype=torch.uint8, device="cuda")
    ms = bench(lambda: fp8_memcpy(a))
    seconds = ms / 1000.0
    return {
        "ms": ms,
        "gb_s": (2 * n_bytes) / 1e9 / seconds,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=[1, 2, 4, 8],
        default=[8],
        help="Fused-kernel encoder-blocks per CTA.",
    )
...
def main() -> None:
    ...
    memcpy = bench_memcpy(n_bytes)
    raw = bench_raw(n_bytes)

    print(
        f"memcpy: {memcpy['ms']:.3f} ms  {memcpy['gb_s']:.1f} GB/s  "
        f"(practical HBM ceiling, 1R+1W)",
        flush=True,
    )
