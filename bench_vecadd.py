...
def bench_fused(n_bytes: int, n_fp8_per_stream: int) -> dict[str, float]:
    ...
    ms = bench(lambda: fp8_vecadd_fused_tans(*args, n_fp8_per_stream))
    seconds = ms / 1000.0
...
def main() -> None:
    ...
    print(f"raw: {raw['ms']:.3f} ms  {raw['gb_s']:.1f} GB/s")
    for n_fp8_per_stream in args.n:
        if n_bytes % n_fp8_per_stream != 0:
            continue
        fused = bench_fused(n_bytes, n_fp8_per_stream)
        print(
